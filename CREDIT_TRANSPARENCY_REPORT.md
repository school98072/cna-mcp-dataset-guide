...
| 表觀維度 | 換算公式 | 換算結果 | 說明 |
| :--- | :--- | :---: | :--- |
| **單次提問表觀扣點** | 180 Credits ÷ 3 次問題 | **60 Credits / 次** | 誤將 Agent 內部多步調用視為單次提問 |
| **全月可用提問次數** | 700 Credits ÷ 60 Credits | **約 11.6 次** | 若每次提問均觸發大迴圈之極端情況 |
| **單次對話金錢成本** | 200 元 ÷ 11.6 次 | **約 17.2 元台幣 / 次** | 傳統黑盒計費產生的直觀恐慌 |
...
| 計費維度 | 換算公式 | 數值結果 | 核心含義 |
| :--- | :--- | :---: | :--- |
| **單點 Credit 價值** | 200 元 ÷ 700 點 | **約 NT$ 0.2857 / 點** | 每點點數之實際法定貨幣價值 |
| **單次 API 檢索真實成本** | 2 點 × NT$ 0.2857 | **約 NT$ 0.57 / 次** | 底層發起 1 次新聞查詢或工具調用之成本 |
| **全月額度折合 API 總次數** | 700 點 ÷ 2 點 | **350 次真實 API 呼叫** | NT$ 200 方案每月可發起的最大 API 請求次數 |

### 兩種完全不同使用型態的壽命分析：

| 使用模式 | 典型使用場景 | 每次提問消耗工具數 | 每次對話點數消耗 | 700 點全月可支撐總次數 | 單次提問真實台幣成本 |
| :--- | :--- | :---: | :---: | :---: | :---: |
| **模式 A：精準對話問答**<br>_(Claude Desktop / ChatGPT / Spark)_ | 日常重大新聞查證、台股盤後焦點事件、單篇新聞精讀、外文譯名查詢 | 1 ～ 2 次 | **2 ～ 4 Credits** | **175 ～ 350 次提問** | **約 NT$ 0.57 ～ 1.14 / 次**<br>（極具競爭力） |
| **模式 B：自主 Agent 深度探勘**<br>_(Antigravity / Cursor / AutoGPT)_ | 宏觀時序統計、37 年資料庫逐年掃描、產業脈絡多輪反覆推理 | 30 ～ 60 次 | **60 ～ 120 Credits** | **5 ～ 10 次巨型任務** | **約 NT$ 17.2 ～ 34.3 / 任務**<br>（需謹慎設限） |

```mermaid
flowchart LR
    subgraph Plan["200 元台幣月費 (700 Credits)"]
        Budget["總額度: 700 點 (折合約 350 次 API 呼叫)"]
    end

    subgraph ModeA["模式 A: 精準對話問答 (Claude / ChatGPT)"]
        A1["每次提問觸發 1~2 次工具"]
        A2["每次對話消耗 2~4 Credits"]
        A3["可支撐 175 ~ 350 次深度提問"]
        A4["每問一次約 0.57 ~ 1.14 元台幣"]
    end

    subgraph ModeB["模式 B: 自主 Agent 深度探勘 (Antigravity / Cursor)"]
        B1["單條指令引發 Agent 迴圈遍歷"]
        B2["單次任務自動呼叫 30~60 次 API"]
        B3["單次任務消耗 60~120 Credits"]
        B4["全月僅能支撐 5 ~ 10 次大型探勘"]
    end
